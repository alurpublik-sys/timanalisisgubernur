...
revoke all on function private.ah_admin_login_impl(text) from public, authenticated;
grant execute on function private.ah_admin_login_impl(text) to anon;
...
as $$
  select * from private.ah_admin_login_impl(p_pin);
$$;
...
create or replace function private.ah_admin_logout_impl()
returns boolean
language plpgsql
volatile
security definer
...
revoke all on function private.ah_admin_logout_impl() from public, authenticated;
grant execute on function private.ah_admin_logout_impl() to anon;
...
set search_path = pg_catalog, private
as $$
  select private.ah_admin_logout_impl();
$$;
...
create policy ah_private_deny_all on private.ah_admin_config for all to public using (false) with check (false);
create policy ah_private_deny_all on private.ah_admin_sessions for all to public using (false) with check (false);
create policy ah_private_deny_all on private.ah_admin_attempts for all to public using (false) with check (false);
